...
import ServicesPost from '../../services/post';
import formErrorMessageShow from '../../utils/form_error_message_show';
import Filters from '../../filters';
import quickPasteHoc from '@/npm/@edu_components_form/file_uploader/quick_paste_hoc';
...
export default class Component extends React.PureComponent {
    ...
    this.columns = [
      {
        ...
        render: (defaultValue, dataSource, {
          elem,
        }) => {
          const uploaderProps = {
            ...
            // 取出props中的getPasteProps,修饰目标组件
            ...!this.props.getPasteProps ? {} : this.props.getPasteProps(elem.title),
          };

          return (
            <ComponentsForm.FileUploader {...uploaderProps} />
          );
        },
        rules: [{
    ...
    this.contentColumns = [
      {
        ...
        render: (text) => {
          return (<ComponentsForm.HtmlBraftEditor className="html_content_editor" content={text} />);
        },
        // extra: '为了更好的编辑效果，请确保屏幕宽度在1300px以上。',
      },
    ];
  }
  ...
  handleSubmitRun = ({ values, preview }) => {
    ...
    let promise;
    if (preview) {
      promise = ServicesPost.preview(formData);
    }
    else if ('update' === this.editInfo.method) {
      promise = ServicesPost.update(this.editInfo.paramsId, formData);
    }
    else {
      promise = ServicesPost.create(formData);
    }
    promise.then((res) => {
